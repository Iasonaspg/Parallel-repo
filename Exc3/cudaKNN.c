...
int main(int argc, char** argv){
    ...
    corr<<<6144,1024>>>(queries,3*Nq);
    corr<<<6144,1024>>>(corpus,3*Nq);
    ...
    int newNq = 8*Nq;
    float* boxesC;
    CHECK(cudaMallocManaged(&boxesC, 3*newNq*sizeof(float)));
    ...
    for (int i=0; i<Nq; i++){
        ...
        int boxC = coX_C + coY_C*d + coZ_C*d*d;
        if ((boxC >= d*d*d) || (boxC < 0)){
            printf("Error, non existing box\n");
            exit(1);
        }
        boxesC[boxC*elements + auxC[boxC]] = corpus[i];
